...
void clusterHelper(int i, std::vector<int>& cluster, std::vector<bool>& processed, const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol) 
{
  processed[i] = true;
  cluster.push_back(i);
  ...
  for(int id:nearby) {
    if(!processed[id])
      clusterHelper(id, cluster, processed, points, tree, distanceTol);
  }
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
  ...
  for(int i=0; i<points.size(); i++) {
    if(!processed[i]) {
      ...
      clusterHelper(i, cluster, processed, points, tree, distanceTol);
